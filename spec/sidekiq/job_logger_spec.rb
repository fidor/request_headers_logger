...
describe RequestHeadersLogger::Sidekiq::JobLogger do
  ...
  context 'when job successfully done' do
    it 'logs start and done' do
      ...
      expect(io.to_a).to eq(
        [
          "[#{request_id}] [sidekiq] [PID: #{pid}] [TID-#{tid}] start\n",
          "[#{request_id}] [sidekiq] [PID: #{pid}] [TID-#{tid}] done: 0.0 sec\n"
        ]
      )
    end
  end
  ...
  context 'when job fails' do
    it 'logs start and fail' do
      expect { job_logger.call(item, 'queue') { raise 'test error' } }.to raise_error(StandardError)
      io.rewind
      expect(io.to_a).to eq(
        [
          "[#{request_id}] [sidekiq] [PID: #{pid}] [TID-#{tid}] start\n",
          "[#{request_id}] [sidekiq] [PID: #{pid}] [TID-#{tid}] fail: 0.0 sec\n"
        ]
      )
    end
  end
